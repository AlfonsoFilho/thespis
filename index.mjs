...
function WorkerPool(url, onMessage) {
    ...
    for (let i = 0; i <= this.maxWorkers; i++) {
        const message = { type: CONTEXT_SET, payload: { currentWorker: i, workers: this.workerList.map(({ id }) => id) } }
...
function ActorSystem(settings) {
    ...
    if (settings.root) {
        pool.postMessage({ type: SPAWN, sender: SYSTEM, payload: { url: settings.root } });
    }
